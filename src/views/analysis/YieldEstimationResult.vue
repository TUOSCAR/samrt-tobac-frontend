...
function updateMap() {
  ...
  // 使用API返回的数据
  const fieldBoundary = yieldResult.value.fieldBoundary;
  const yieldMapData = yieldResult.value.yieldMapData;
  ...
  // 如果API返回了产量分布数据，绘制产量热力图
  if (yieldMapData && yieldMapData.length > 0) {
    console.log('使用API返回的产量分布数据，共', yieldMapData.length, '个点');
    
    // 创建热力图数据
    const heatData = yieldMapData.map((point: any) => {
      // 归一化产量值到0-1范围，用于热力图强度
      const yieldValue = point.yield || 0;
      const normalizedValue = Math.min(1, Math.max(0, (yieldValue - 150) / 130)); // 假设150-280范围
      
      return [
        point.lat, 
        point.lng, 
        normalizedValue // 热力图强度
      ];
    });
    
    // 创建热力图图层，使用产量值对应的颜色渐变
    const heat = L.heatLayer(heatData, {
      ...
      gradient: {
        0.0: yieldColors.low,
        0.5: yieldColors.medium,
        1.0: yieldColors.high
      }
    }).addTo(map);
    
    hasValidData = true;
  } else {
    console.warn('使用生成的随机产量数据，因为API未返回yieldMapData数据');
