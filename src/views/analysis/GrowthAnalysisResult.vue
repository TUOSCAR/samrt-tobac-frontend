...
// 更新地图
function updateMap() {
  if (!map || !growthResult.value.tifPath) {
    return
  }
  ...
  // 模拟数据，实际项目中需要从API获取TIF图片或GeoJSON数据
  const fieldBoundary = generateMockFieldBoundary()
  ...
  // 设置地图视图范围
  map.fitBounds(L.geoJSON(fieldBoundary).getBounds(), { padding: [20, 20] })
}
...
function generateMockGridCells(fieldBoundary: any, gridSize: number) {
  ...
  for (let i = 0; i < gridSize; i++) {
    for (let j = 0; j < gridSize; j++) {
      // 计算单元格的边界
      const southWest = [minLat + i * cellHeight, minLng + j * cellWidth]
      const northEast = [minLat + (i + 1) * cellHeight, minLng + (j + 1) * cellWidth]
